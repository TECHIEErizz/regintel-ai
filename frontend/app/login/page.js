...
export default function LoginPage() {
    ...
    const handleLogin = async (e) => {
        ...
        // For demo, anything works, but we check if user exists in local storage
        const users = JSON.parse(localStorage.getItem("regintel_registered_users") || "[]");
        const user = users.find(u => u.email === email && u.password === password) || (email === "demo@example.com" && password === "password");

        if (user) {
            localStorage.setItem("regintel_user", JSON.stringify({ email, name: user.name || "Demo User" }));
            window.location.href = "/dashboard";
        } else {
            alert("Invalid credentials. Use demo@example.com / password or register a new account.");
        }
        setLoading(false);
    };
            ...
            <div className="w-full max-w-md relative">
                ...
                <div className="bg-white/5 border border-white/10 backdrop-blur-2xl rounded-3xl p-8 shadow-2xl">
                    ...
                    <p className="mt-8 text-center text-slate-400 text-sm">
                        ...
                        <button type="button" onClick={() => window.location.href = '/register'} className="text-violet-400 font-bold hover:text-violet-300 transition-colors underline-offset-4 hover:underline">
                            Create an account
                        </button>
                    </p>
                </div>
                ...
                <div className="mt-8 text-center">
                    <p className="text-xs text-slate-600 uppercase tracking-widest font-semibold">Demo Credentials</p>
                    <p className="text-sm text-slate-500 mt-2">demo@example.com / password</p>
                </div>
            </div>
        </div>
    );
